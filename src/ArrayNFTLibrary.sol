...
library ArrayNFTLibrary {
    ...
    function toArray(ITheFedz self) internal view returns (uint256[] memory) {
        return _toArray(self);
    }

    function toArrayWithShuffle(ITheFedz self, uint256 random) internal view returns (uint256[] memory) {
        uint256[] memory array = _toArray(self);
        array.shuffle(random);
        return array;
    }

    function _toArray(ITheFedz self) internal view returns (uint256[] memory) {
